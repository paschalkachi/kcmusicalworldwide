    ...
    <div class="space-y-6">
        <x-common.component-card>
            ...
            <div class="lg:hidden flex items-center justify-between mb-4">
                <h2 class="text-base font-semibold text-gray-900 dark:text-white">Latest Categories</h2>
            ...
            <div class="hidden lg:block overflow-x-auto">
                <h2 class="text-lg font-semibold mb-4 text-gray-900 dark:text-white">Latest Categories</h2>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                                ...
                                <td class="px-4 py-4 border-r border-gray-200 dark:border-gray-700">
                                    <div class="flex items-center gap-3">
                                        <img 
                                            src="{{ $category->image ? asset('uploads/categories/' . $category->image) : asset('images/logo/logo-icon.png') }}"
                                ...
                                <td class="px-4 py-4 border-r border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white">{{ $category->slug }}</td>
                                <td class="px-4 py-4 border-r border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white">{{ $category->products_count ?? 0 }}</td>
            ...
            <div class="lg:hidden space-y-4">
                    ...
                    <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-category-white dark:bg-category-dark p-4 shadow-sm hover:shadow-md transition">
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="font-semibold text-gray-900 dark:text-white leading-tight truncate" title="{{ $category->name }}">
                                    {{ $category->name }}
                                </p>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5 truncate" title="{{ $category->slug }}">
                                    {{ $category->slug }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 text-sm text-gray-700 dark:text-gray-300 border-t border-gray-100 dark:border-gray-700 pt-3">
                            <div><span class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Slug</span>
                                <span class="text-gray-900 dark:text-white font-medium">{{ $category->slug }}</span></div>
                            <div><span class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Products</span>
                                <span class="text-gray-900 dark:text-white font-medium">{{ $category->products_count ?? 0 }}</span></div>
                        </div>
                        ...
                        <div class="mt-4 grid grid-cols-2 gap-3">
                            ...
                            <form action="{{ route('categories.destroy', $category) }}" method="POST" class="inline-block w-full">
                                @csrf
                                @method('DELETE')
                                <button
                ...
                @empty
                    <p class="text-center text-gray-500 dark:text-gray-400 py-6">No categories found.</p>
                @endforelse
