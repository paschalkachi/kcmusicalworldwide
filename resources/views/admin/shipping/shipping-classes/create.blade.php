...
@section('content')
<x-common.page-breadcrumb pageTitle="Add New Shipping Class" />

<div class="space-y-6">
    <!-- Display existing shipping classes -->
    @if(isset($existingClasses) && $existingClasses->count() > 0)
    <x-common.component-card title="Existing Shipping Classes">
        ...
        <p class="mt-2 text-sm text-blue-600">If your new shipping class range overlaps with existing ranges, the system will automatically adjust the overlapping ranges.</p>
    </x-common.component-card>
    @endif

    <x-common.component-card title="Add New Shipping Class">
        @if($errors->any())
        ...
        <form method="POST" action="{{ route('shipping-classes.store') }}" id="shipping-class-form">
            @csrf
...
<script>
    // Enable load factor input when min/max units are entered
    document.getElementById('min-units-input').addEventListener('change', function() {
        enableLoadFactorIfNeeded();
    });
    
    document.getElementById('max-units-input').addEventListener('change', function() {
        enableLoadFactorIfNeeded();
    });
    
    function enableLoadFactorIfNeeded() {
        const minUnits = document.getElementById('min-units-input').value;
        const maxUnits = document.getElementById('max-units-input').value;
